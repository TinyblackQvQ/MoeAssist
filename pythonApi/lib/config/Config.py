...
class Cfglib(ICfglib):
    pipInstallSource = {"name": "pipInstallSource",
                        "type": "str"}
...
class Config:
    ...
    def __init__(self, file_path=mapath.acurdir("../config.cfg"), cfglib_=Cfglib):
        ...
        for line in self.fileHandle.readlines():
            ...
            for cfg_ in self.cfglib:
                if cfg_.name == name:
                    if value is None:
                        pass
                    elif cfg_.value.get("type") == "float":
                        value = float(value)
                    elif cfg_.value.get("type") == "bool":
                        value = bool(value)
                    elif cfg_.value.get("type") == "int":
                        value = int(value)
    ...
    def set(self, config: ICfglib, value):
        ...
        :return: None
        """
        if str(type(value)).find(config.value.get("type")) == -1:
